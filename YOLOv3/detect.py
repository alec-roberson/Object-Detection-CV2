...
if __name__ ==  '__main__':
    args = arg_parse() # parse the arguments!
    
    scales = args.scales # get the scales to use
    
    # convert all arguments to the right data types
    images = args.images
    ...
    write = False # have we written output yet
    ...
    for batch in im_batches:
        ...
        if type(prediction) == int: # if prediction = 0 (no obj found)
            i += 1 # increment counter
            continue # loop back

            
        #Add the current batch number
        prediction[:,0] += i*batch_size # add the current batch number to the prediction
        
    
            
          
        if not write:
            output = prediction # set the output variable
            write = 1 # set write to 1 for nexttime
        else:
            # otherwise add the prediction to the current output
    ...
    for a, b in zip(det_names, orig_ims):
        cv2.imwrite(a, b)
    print(det_names)
