''' dataparser.py
this file contains the DataParser class which can be used to load data from a
directory, split it into training and testing data, and save it to binary files
which can then be read by the DataSet class.
'''
# +++ IMPORTS
import os
import cv2
...
class DataParser:
    ...
    def __init__(self, raw_data_path, class_file, resize_dim, suppress_warnings=False):
        ...
        # load all the data
        fnames = get_matching_files(raw_data_path, suppress_warnings=suppress_warnings)
        self.data = np.concatenate((
            load_images(raw_data_path, fnames, resize_dim),
            load_labels(raw_data_path, fnames)), axis=1)
        self.resize_dim = resize_dim
    
    def save_data(self, train_fn, test_fn, test_pct=0.1, shuffle=True):
        ''' save data method
